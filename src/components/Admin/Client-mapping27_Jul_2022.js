...
class Clientmapping extends Component {
    ...
    selectdata = (e) => {
      ...
      var todate = $("#to").val();
        ...
        axios.post('/api/searchclient',{pan:inputPan,name: inputName,fromdate:fromdate,todate:todate})
        .then(res => {
                ...
                onClick: () => {
                  $.ajax({
                    ...
                    success: function (res1) {
                      ...
                      {res1.map((item, index) => (
                        foliodetail += "<option value='"+item.PAN+"_"+item.INVNAME+"_"+item.GUARDPAN+"_"+item.ADD1+"_"+item.ADD2+"_"+item.ADD3+"'><span class='client-name'>"+item.INVNAME+"</span>  "+item.PAN+"  "+item.ADD1+"  "+item.ADD2+"  "+item.ADD3+"</option>"   
                      ))}
   ...
   const values = {
    ...
    const data = {
        ...
        rows:this.state.searchdata.map(item => {      
          return{
            CHECK:<input type="checkbox" class="clientdata" id={item._id} name={item.PAN} value={item.PAN+"_"+item.INVNAME+"_"+item.GPAN+"_"+item.ADD1+"_"+item.ADD2+"_"+item.ADD3} checked={this.state.checkedBoxes.find((p) => p.id === item._id)} onChange={(e) => this.toggleCheckbox(e, item)}/>,
