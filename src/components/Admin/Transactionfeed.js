...
function Transactionfeed() {
  ...
  const processData = (dataString, file) => {
    ...
    console.log("dataString=",dataStringLines.length)
    const headers = dataStringLines[0].split(/,(?![^"]*"(?:(?:[^"]*"){2})*[^"]*$)/);
    console.log("headers=",headers)
    const list = []; const newlist = []
    for (let i = 1; i < dataStringLines.length; i++) {
      console.log(i +"====", dataStringLines.length);
    ...
    if (lastTwo === '2A') {
      Axios.post('http://localhost:3001/api/uploadtranscams2atest',
        ...
        if (res.data.status === 200) {
          ...
          settotaluploaded(res.data.data.totaluploaded);
          // console.log( "res", res.data.data.totaluploaded)
          settotalexcel(res.data.data.totalexcel);
          setloop(res.data.data.loop);
          setnewclient(res.data.data.newclient); 
          console.log("resnew-", res.data.data.data)
                        ...
                        <div className="card">
                          ...
                          <div id="collapseTwo" className="collapse" aria-labelledby="headingTwo" data-parent="#accordionExample">
                            <div className="card-body">
                              ...
                              <div><label htmlFor="exampleInputFile" >Totaluploaded=</label>{totaluploaded+1}</div> 
                          <div><label htmlFor="exampleInputFile" >Totalexcel=</label>{totalexcel}</div>                                                          
                          <div><label htmlFor="exampleInputFile" >Toop=</label>{loop}</div>
                          <div><label htmlFor="exampleInputFile" >Newclient=</label>{newclient}</div>                                                            
                          </div>
                                <label htmlFor="exampleInputFile">File input</label>
                                <div className="input-group">
