...
async function uploadtransactionkarvyold(exceldata) {
    try {
        const totaluploaded = [];
        var newclientarray = [];
        var j = '';
            ...
            if (exceldata[i].TD_ACNO != undefined) {
                ...
                if (foliodata === null || foliodata[0] === undefined) {
                    ...
                    for (var j = 0; j < foliodata.length; j++) {
                        ...
                        try {
                            const filter = { TD_TRNO: exceldata[i].TD_TRNO, UNQNO: exceldata[i].UNQNO, NEWUNQNO: exceldata[i].NEWUNQNO, TD_ACNO: exceldata[i].TD_ACNO, FMCODE: exceldata[i].FMCODE };
                            const updateDoc = {
                                $set: {
                            ...
                            transk.find({ TD_TRNO: exceldata[i].TD_TRNO, UNQNO: exceldata[i].UNQNO, NEWUNQNO: exceldata[i].NEWUNQNO, TD_ACNO: exceldata[i].TD_ACNO, FMCODE: exceldata[i].FMCODE }, function (err, data) {
...
async function uploadtransactionkarvy(data1) {
    try {
        var exceldata = [];
        var data = data1;
        var newdata = data1;
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].TD_UNITS < 0 && data[i].TD_AMT < 0) {
                positivedata.push(data[i])
            }
        }
        console.log("positivedata=", positivedata.length)
        for (var k = 0; k < positivedata.length; k++) {
            var p = 0;
            for (var j = k; j < newdata.length; j++) {
                if (p === 0) {
                    if (positivedata[k].TD_UNITS === newdata[j].TD_UNITS && positivedata[k].TD_AMT === newdata[j].TD_AMT) {
                        console.log("negative=", positivedata[k].TD_UNITS, newdata[j].TD_UNITS, positivedata[k].TD_AMT, newdata[j].TD_AMT)
                        p = 1;
                        newdata[j] = [];
                    }
                }
        ...
        for (var k = 0; k < positivedata.length; k++) {
            ...
            for (var j = k; j < newdata.length; j++) {
                if (p === 0) {
                    if (Math.abs(positivedata[k].TD_UNITS) == newdata[j].TD_UNITS && Math.abs(positivedata[k].TD_AMT) == newdata[j].TD_AMT) {
                        console.log("positive=", positivedata[k].TD_UNITS, newdata[j].TD_UNITS, positivedata[k].TD_AMT, newdata[j].TD_AMT)
                        p = 1;
                        newdata[j] = [];
                    }
                }
        ...
        for (var i = 0; i < newdata.length; i++) {
            if (newdata[i].length === undefined) {
                exceldata.push(newdata[i]);
            }
        }
            ...
            if (exceldata[i].TD_ACNO != undefined) {
                ...
                if (foliodata === null || foliodata[0] === undefined) {
                    ...
                    for (var j = 0; j < foliodata.length; j++) {
                        ...
                        console.log("data not null=", exceldata[i].TD_ACNO, exceldata[i].TD_TRNO, exceldata[i].PAN1)
                        try {
                            const filter = { TD_TRNO: exceldata[i].TD_TRNO, UNQNO: exceldata[i].UNQNO, TD_ACNO: exceldata[i].TD_ACNO, FMCODE: exceldata[i].FMCODE };
                            const updateDoc = {
                                $set: {
                                    FMCODE: exceldata[i].FMCODE,
                                    TD_FUND: exceldata[i].TD_FUND,
                            ...
                            console.log(`Updated ${result.n} documents`);
                            transk.find({ TD_TRNO: exceldata[i].TD_TRNO, UNQNO: exceldata[i].UNQNO,  TD_ACNO: exceldata[i].TD_ACNO, FMCODE: exceldata[i].FMCODE }, function (err, data) {
                                //console.log("ddddddd====",data[0]._doc)
                                totaluploaded.push(data[0]._doc)
                                console.log("i=", i, "totaluploaded.length=", totaluploaded.length, "exceldata=", exceldata.length, "newclient=", newclientarray.length)
                                //console.log("totaluploaded=",totaluploaded)       
                ...
                if (exceldata.length === i + 1) {
                    ...
                    resdata = {
                        ...
                        loop: i,
                        newclient: newclientarray.length
...
async function uploadtransactioncamsold(exceldata) {
    try {
        const totaluploaded = [];
        var newclientarray = [];
        for (var i = 0; i < exceldata.length; i++) {
...
async function uploadtransactioncams(data1) {
    try {
        var exceldata = [];
        var data = data1;
        var newdata = data1;
        var positivedata = [];
        for (var i = 0; i < data.length; i++) {
            if (data[i].UNITS < 0 && data[i].AMOUNT < 0) {
                positivedata.push(data[i])
            }
        }
        console.log("positivedata=", positivedata.length)
        for (var k = 0; k < positivedata.length; k++) {
            ...
            for (var j = k; j < newdata.length; j++) {
                if (p === 0) {
                    if (positivedata[k].UNITS === newdata[j].UNITS && positivedata[k].AMOUNT === newdata[j].AMOUNT) {
                        console.log("negative=", positivedata[k].UNITS, newdata[j].UNITS, positivedata[k].AMOUNT, newdata[j].AMOUNT)
                        p = 1;
                        newdata[j] = [];
                    }
                }
        ...
        for (var k = 0; k < positivedata.length; k++) {
            ...
            for (var j = k; j < newdata.length; j++) {
                if (p === 0) {
                    if (Math.abs(positivedata[k].UNITS) == newdata[j].UNITS && Math.abs(positivedata[k].AMOUNT) == newdata[j].AMOUNT) {
                        console.log("positive=", positivedata[k].UNITS, newdata[j].UNITS, positivedata[k].AMOUNT, newdata[j].AMOUNT)
                        p = 1;
                        newdata[j] = [];
                    }
                }
...
async function uploadfoliocams(exceldata) {
    ...
    try {
        const totaluploaded = [];
        // console.log(totaluploaded="exceldata")        
        var newclientarray = []        
        for (var i = 0; i < exceldata.length; i++) {
                ...
                if (exceldata[i].GUARD_PAN === "") {
                    ...
                    if (userdata === null || userdata[0] === undefined) {
                        ...
                        try {
                            ...
                            folioc.find({ FOLIOCHK: exceldata[i].FOLIOCHK, PRODUCT: exceldata[i].PRODUCT, FOLIO_DATE: moment(new Date(exceldata[i].FOLIO_DATE)).format("YYYY-MM-DD") }, function (err, data) {
                                ...
                                // let dd = {
                                //     up_i:1,
                                //     up_length:totaluploaded.length
                                // }
                                // console.log("newclient=",newclientarray.length)       
                            });
                        } catch (error) {
                            console.log(`Error found. ${error}`)
                        }
                    ...
                    if (exceldata.length === i + 1) {
                        resdata = {
                            ...
                            message: 'Data uploaded',
                            data: newclientarray,
                            // upData1: totaluploaded,
                            // upData2: exceldata.length,
                            // upData:totaluploaded.length
